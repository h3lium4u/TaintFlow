# Consolidated TaintFlow+ Project & Diagnosis Reports

This file consolidates multiple critical reports for the TaintFlow+ project, including dataset diagnosis, overall project status, V4 feature specifications, Round 3 remediation notes, and feature correlation stats.

## Table of Contents
- [DATASET_DIAGNOSIS_REPORT.md](#dataset_diagnosis_reportmd)
- [PROJECT_STATUS.md](#project_statusmd)
- [FEATURE_MAPPING_V4.md](#feature_mapping_v4md)
- [ROUND3_REMEDIATION_REPORT.md](#round3_remediation_reportmd)
- [FEATURE_CORRELATION_REPORT.md](#feature_correlation_reportmd)
...
# FEATURE_MAPPING_V4.md

# Feature Mapping V4 Specification

This document defines the 48 dense semantic features (feat_0 to feat_47) extracted by the TaintFlow feature engine (V4 release). These features are classified into 4 major subsystems: Taint Subsystem, CFG Subsystem, Symbol Subsystem, and Rule Subsystem.
...
| :--- | :--- | :---: | :--- |
| `feat_0` | `source_count` | Continuous | Count of source keywords (e.g. `request.args`, `input(`, `sys.argv`, etc.). |
| `feat_1` | `sink_count` | Continuous | Count of sink keywords (e.g. `execute`, `query`, `run`, `popen`, `system`, etc.). |
| `feat_2` | `validated_paths` | Continuous | Count of verified control/data flow paths from a tainted source to a sink. |
| `feat_3` | `source_categories` | Continuous | Count of distinct taint source categories (web params, environment, user input, cookies/headers). |
| `feat_4` | `longest_path` | Continuous | DFS longest path length from CFG entry to exit (clipped to 100.0). |
| `feat_5` | `sanitizers` | Continuous | Count of sanitizer keywords (e.g. `strip`, `escape`, `sanitize`, `encode`, etc.). |
| `feat_6` | `unsanitized_paths` | Binary | Binary indicator (1.0 if any taint paths exist and sanitizers count < validated paths count). |
| `feat_7` | `alias_transitions` | Continuous | Count of actual variable alias transitions during taint propagation. |
| `feat_8` | `container_props` | Continuous | Count of container propagation method calls (e.g. `append`, `add`, `put`, `extend`). |
| `feat_9` | `path_source_ratio` | Continuous | Ratio of validated paths to (source count + 1.0). |
| `feat_10` | `tainted_symbol_ratio`| Continuous | Ratio of tainted symbols to (total symbols count + 1.0). |
| `feat_11` | `max_alias_depth` | Continuous | Observed maximum variable alias propagation depth. |
...
| Feature | Name | Type | Description |
| :--- | :--- | :---: | :--- |
| `feat_12` | `cfg_nodes` | Continuous | Count of nodes in the control flow graph (clipped to 1000.0). |
| `feat_13` | `cyclomatic_complexity`| Continuous| Cyclomatic complexity: `branches + loops + exceptions + 1.0` (clipped to 50.0). |
| `feat_14` | `branch_node_density` | Continuous | Ratio of branch nodes / 2.0 if validated paths > 0 else 0.0. |
| `feat_15` | `loop_nodes` | Continuous | Raw loop node count in the CFG. |
| `feat_16` | `exception_node_density`| Continuous| Ratio of exception catch nodes / 2.0 if validated paths > 0 else 0.0. |
| `feat_17` | `cfg_density` | Continuous | Ratio of CFG edges to (CFG nodes + 1.0). |
| `feat_18` | `return_nodes` | Continuous | Count of Return nodes (explicit exit statements) in the CFG. |
| `feat_19` | `branch_indicator` | Binary | 2.0 if branch nodes > 0 else 0.0. |
| `feat_20` | `loop_indicator` | Binary | 2.0 if loop nodes > 0 else 0.0. |
| `feat_21` | `exception_nodes` | Continuous | Raw exception catch nodes count in the CFG. |
| `feat_22` | `exception_indicator` | Binary | 1.0 if try/catch block exists in CFG, else 0.0. |
| `feat_23` | `path_count` | Continuous | Distinct acyclic path count from entry to exit (capped at 50.0). |
...
| Feature | Name | Type | Description |
| :--- | :--- | :---: | :--- |
| `feat_24` | `total_symbols` | Continuous | Total symbol definitions tracked in the symbol table. |
| `feat_25` | `scopes_count` | Continuous | Count of scopes/nesting levels (e.g. classes, functions). |
| `feat_26` | `sink_imports` | Continuous | Count of imported symbols matching sink keywords. |
| `feat_27` | `sanitizer_imports` | Continuous | Count of imported symbols matching sanitizer keywords. |
| `feat_28` | `parameters` | Continuous | Count of function/method parameters. |
| `feat_29` | `fields` | Continuous | Count of class field/attributes (e.g. variables prefixed with `self.`). |
| `feat_30` | `imports` | Continuous | Count of imported modules or module objects. |
| `feat_31` | `nested_scopes` | Binary | 1.0 if scopes count > 1, else 0.0. |
...
| Feature | Name | Type | Description |
| :--- | :--- | :---: | :--- |
| `feat_32` | `findings_indicator` | Binary | 1.0 if any vulnerability findings are detected, else 0.0. |
| `feat_33` | `web_finding` | Binary | 1.0 if any finding details contain "args", "form", or "json", else 0.0. |
| `feat_34` | `multi_source_finding`| Binary | 1.0 if multiple distinct taint sources are detected, else 0.0. |
| `feat_35` | `env_finding` | Binary | 1.0 if any finding details contain "env" or "getenv", else 0.0. |
| `feat_36` | `cwe_89_finding` | Binary | 1.0 if SQL Injection finding (CWE-89) exists, else 0.0. |
| `feat_37` | `cwe_78_finding` | Binary | 1.0 if Command Injection finding (CWE-78) exists, else 0.0. |
| `feat_38` | `cwe_22_finding` | Binary | 1.0 if Path Traversal finding (CWE-22) exists, else 0.0. |
| `feat_39` | `cwe_918_finding` | Binary | 1.0 if Server-Side Request Forgery finding (CWE-918) exists, else 0.0. |
| `feat_40` | `cwe_502_finding` | Binary | 1.0 if Deserialization finding (CWE-502) exists, else 0.0. |
| `feat_41` | `cwe_327_finding` | Binary | 1.0 if Broken/Weak Cryptography finding (CWE-327) exists, else 0.0. |
| `feat_42` | `escape_indicator` | Binary | 1.0 if "escape", "sanitize", or "replace" keywords are in code, else 0.0. |
| `feat_43` | `quote_indicator` | Binary | 1.0 if "quote" or "escape" keywords are in code, else 0.0. |
| `feat_44` | `canonical_indicator` | Binary | 1.0 if "canonicalize", "abspath", or "realpath" are in code, else 0.0. |
| `feat_45` | `regex_indicator` | Binary | 1.0 if "match", "regex", or "re.sub" keywords are in code, else 0.0. |
| `feat_46` | `validated_finding` | Binary | 1.0 if findings count > 0 and validated paths count > 0, else 0.0. |
| `feat_47` | `complex_finding` | Binary | 1.0 if cyclomatic complexity > 2.0 and "unsanitized" is in findings, else 0.0. |


---

# ROUND3_REMEDIATION_REPORT.md

# Round 3 Remediation Report

This report documents the verification and correctness remediation of the 48-feature dataset extraction pipeline for Round 3. All requested Round 3 modifications have been successfully implemented.

## 1. Remediation Scope & Verification
- **feat_7 (Actual Alias Transition Count)**: Updated to count actual alias transition events during alias propagation rather than the number of active tainted symbols.
- **feat_41 (CWE-327 Findings Only)**: Derives purely from rule-based findings associated with CWE-327 (broken or weak cryptographic algorithms). All callee-name keyword fallback checks in feature calculation have been removed.
- **DFS Abort Handling**: DFS longest path (`get_longest_path` / `get_longest_path_py`) and average path (`get_average_path_length` / `get_average_path_length_py`) handle aborts (`steps > 5000`) by returning the best-so-far path metric and never returning `0` on abort (using a fallback of at least `1.0`).
- **Python/Rust Parity**: Replaced the hardcoded `10` max alias depth in Python with a configurable `max_alias_depth` matching the Rust `TaintEngine` configuration.

## 2. Dataset Overview
- **Row Count**: 8631 samples
- **Feature Count**: 48 features (exactly 48 features, feat_0 to feat_47)
- **Class Distribution**:
  - **Safe (Class 0)**: 4551 (52.73%)
  - **Vulnerable (Class 1)**: 4080 (47.27%)

## 3. Validation Checklist
- **No Constant Columns**: True (Zero variance count: 0)
- **No NaN/Inf Values**: True
- **Exact Collinearity Pairs**: 0 pairs detected.
- **Path Separation (feat_3 != feat_4)**: True


---

# FEATURE_CORRELATION_REPORT.md

# Feature Correlation Report (Final)

## 1. Summary

| Metric | Value |
|--------|-------|
| Total feature pairs analyzed | 1128 |
| Pairs with correlation > 0.95 | 0 |
| Exact collinearity pairs (1.0) | 0 |

## 2. High Correlation Pairs (> 0.95)

No feature pairs have correlation > 0.95. [PASS]

## 3. Exact Collinearity (1.0)

No exact collinearity pairs exist. [PASS]

## 4. Target Pair Status
