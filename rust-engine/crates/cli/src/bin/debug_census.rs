...
fn main() {
    ...
    for entry in &vuln_samples {
        ...
        program.source_files.insert(filename.clone(), entry.code.clone());
        ...
        let mut detected = semantic_violations.iter().any(|c| c.replace("CWE-", "") == entry.cwe.replace("CWE-", ""));
        ...
        if !detected {
            fn_count += 1;
            println!("FN {}: test_id={}, expected_cwe={}", fn_count, entry.test_id, entry.cwe);
        }
    }
