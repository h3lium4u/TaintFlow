...
fn main() {
    ...
    for fact in &engine.tainted_facts {
        ...
        let inst = node.instruction_id.and_then(|id| program.instructions.get(&id));
        println!("  Fact: node={}, var={}, sanitized_for={:?}, inst={:?}", 
                 fact.node_id, fact.var, fact.sanitized_for, inst.map(|i| &i.kind));
    }
}
