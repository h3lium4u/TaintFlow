...
fn main() {
    ...
    println!("=== REFINEMENTS FOR 00099 ===");
    for (i, refi) in refinements.iter().enumerate() {
        let flow = &facts.taint_flows[refi.flow_index];
        println!(
            "Refinement {}: flow_index={}, sink_var={}, status={:?}, reason={}",
            i, refi.flow_index, flow.sink_var, refi.status, refi.reason
        );
    }
}
