...
fn main() {
    ...
    gst.load_file(&mut program, &target_code, "paddle/hub.py", &target_lang)
        .unwrap();
    gst.load_file(
        &mut program,
        &sibling_code,
    ...
    for (&method_id, method) in &program.methods {
        ...
        if method.name == "list"
            || method.name == "_git_archive_link"
            || method.name == "get_path_from_url"
            || method.name == "_wget_download"
            || method.name == "_download"
            || method.name == "_parse_repo_info"
        {
            println!(
                "Method '{}' ID: {:?}, parameters: {:?}",
                method.name, method_id, method.parameters
            );
    ...
    for fact in &engine.tainted_facts {
        ...
        let file_path = gst
            ...
            .unwrap_or("unknown");
        println!(
            "  Fact: var='{}' node={} (method='{}' in {})",
            fact.var, fact.node_id, method.name, file_path
        );
