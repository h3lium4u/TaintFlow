...
fn main() {
    ...
    for fact in &engine.tainted_facts {
        ...
        let inst_str = node
            .instruction_id
            .and_then(|inst_id| program.instructions.get(&inst_id))
            .map_or("".to_string(), |inst| format!("{:?}", inst.kind));
        println!(
            "  Seed: node={}, var={}, inst={}",
            fact.node_id, fact.var, inst_str
        );
    }
    ...
    for fact in &engine.tainted_facts {
        ...
        let inst_str = node
            ...
            .map_or("".to_string(), |inst| format!("{:?}", inst.kind));
        println!(
            "  Fact: node={}, var={}, inst={}, sanitized={:?}, source_domain={:?}",
            fact.node_id, fact.var, inst_str, fact.sanitized_for, fact.source_domain
        );
    }
}
