...
impl MockPathSolver {
    pub fn evaluate_flow(
        ...
        // ── CE-D: MLflow Internal Tracking/Artifact URIs (CWE-918/CWE-22) ──────
        //
        // Root cause: MLflow artifact repository paths and server tracking URIs
        // (like `tracking_uri`, `artifact_uri`) are set from workspace configs or env
        // vars, not from untrusted HTTP clients. Seeding them triggers CWE-918/CWE-22.
        //
        // Evidence gate: `import mlflow` or `from mlflow` must be present.
        if (flow.cwe == taint::CWE::CWE918 || flow.cwe == taint::CWE::CWE22)
            && (any_source_contains("import mlflow")
                || any_source_contains("from mlflow"))
        {
            let src_lower = clean_source_var.to_lowercase();
            let is_mlflow_uri = src_lower.contains("tracking_uri")
                || src_lower.contains("artifact_uri")
                || src_lower.contains("registry_uri")
                || src_lower.contains("model_uri")
                || src_lower.contains("get_tracking_uri")
                || src_lower.contains("get_artifact_uri");
            if is_mlflow_uri {
                return PathRefinement {
                    flow_index,
                    status: FeasibilityStatus::Infeasible,
                    reason: format!(
                        "CE-D: MLflow configuration URI '{}' is a workspace config, \
                         not user-controlled HTTP input — suppressed as false positive",
                        clean_source_var
                    ),
                };
            }
        ...
        // ── CE-E: GitPython Repository Directory Path Escape (CWE-22) ───────────
        //
        // Root cause: GitPython paths pointing to repo folders (like `.git`,
        // `git_dir`, `working_dir`) are generated by local repository metadata inspection
        // rather than path traversal inputs.
        //
        // Evidence gate: `import git` or `from git` must be present.
        if flow.cwe == taint::CWE::CWE22
            && (any_source_contains("import git")
                || any_source_contains("from git"))
        {
            let src_lower = clean_source_var.to_lowercase();
            let is_git_path = src_lower.contains("git_dir")
                || src_lower.contains("working_dir")
                || src_lower.contains("working_tree_dir")
                || src_lower.contains(".git")
                || src_lower.contains("repo.path");
            if is_git_path {
                return PathRefinement {
                    flow_index,
                    status: FeasibilityStatus::Infeasible,
                    reason: format!(
                        "CE-E: GitPython repository path variable '{}' is derived from git metadata, \
                         not user traversal input — suppressed as false positive",
                        clean_source_var
                    ),
                };
            }
