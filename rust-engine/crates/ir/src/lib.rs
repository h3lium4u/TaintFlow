        ...
        match &node.kind {
            ...
            NodeKind::VariableDeclarator => {
                ...
                if node.children.len() >= 2 {
                    ...
                    if init.kind == NodeKind::CallExpression {
                        let callee = extract_callee_name(init);
                        let mut args = Vec::new();
                        if let Some(arg_list) = init.children.iter().find(|c| {
                            let t = match &c.kind {
                                NodeKind::Unknown(typ) => typ.as_str(),
                                _ => "",
                            };
                        ...
                        let mut nested_calls = Vec::new();
                        find_call_expressions(init, &mut nested_calls);
                        for call_node in nested_calls {
                            self.collect_statements(&call_node, insts);
                        }
