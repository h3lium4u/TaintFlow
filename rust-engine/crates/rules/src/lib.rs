...
impl RuleEngine {
    ...
    fn is_path_traversal_guarded(file_path: &str, var_name: &str, target_line: usize) -> bool {
        if let Ok(content) = std::fs::read_to_string(file_path) {
            ...
            let start_idx = if target_line > 50 {
                target_line - 50
            } else {
                0
            };
            ...
            for idx in start_idx..=end_idx {
                ...
                for part in &parts {
                    ...
                    let contains_is_none = (line.contains("is none")
                        || line.contains("is not none")
                        || line.contains("!= none")
                        || line.contains("== none")
                        || line.contains("!= null")
                        || line.contains("== null"))
                    ...
                    let contains_dotdot = line.contains("..")
                        && (line.contains(" in ")
        ...
        } = &node.kind
        {
            for (idx, arg) in arguments.iter().enumerate() {
                if let Some((name, state)) = self.get_tainted_identifier(arg, taint_engine) {
                    ...
                    if let Some(c) = cwe {
                        if c == CWE::CWE22
                            && Self::is_path_traversal_guarded(
                                file_path,
                                &name,
    ...
    pub fn evaluate_call_pattern_rules(&mut self, file_path: &str, node: &NormalizedNode) {
        ...
        } = &node.kind
        {
            ...
            let weak_hash_algos = ["md5", "sha1", "sha-1", "md4", "md2"];

            let weak_cipher_algos = ["rc4", "des", "3des", "blowfish", "arcfour", "rc2"];

            let all_weak_algos: Vec<&str> = weak_hash_algos
                .iter()
                .chain(weak_cipher_algos.iter())
                .copied()
                .collect();
            ...
            if is_weak {
                ...
                let is_hash = weak_hash_algos.iter().any(|&algo| {
                    callee_lower.contains(algo)
                        || arguments
                            .iter()
                            .any(|a| a.raw.to_lowercase().contains(algo))
                });
                ...
                if is_hash {
                    ...
                    self.push_finding(Finding {
                        ...
                        description: format!(
                            "Usage of weak/broken cryptographic hash method '{}'.",
                            callee
                        ),
