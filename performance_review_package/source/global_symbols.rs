...
pub struct MethodInfo {
    ...
    #[serde(default)]
    pub local_types: HashMap<String, Vec<(usize, usize, String)>>,
}
        ...
        match node_type {
            ...
            "method_declaration" | "constructor_declaration" => {
                ...
                if let Some(body_node) = node.children.iter().find(|c| {
                    ...
                    let mut local_types: HashMap<String, Vec<(usize, usize, String)>> = HashMap::new();
                    for param in &params_copy {
                        let parts: Vec<&str> = param.split_whitespace().collect();
                        if parts.len() == 2 {
                            let type_name = normalize_java_type(parts[0]);
                            let name = parts[1].to_string();
                            local_types.entry(name).or_default().push((node.span.start_line, body_node.span.end_line, type_name));
                        }
                    }
                    extract_java_local_types(body_node, body_node.span.end_line, &mut local_types);

                    if let Some(method_info) = self.program_index.methods.get_mut(&method_id) {
                        method_info.local_types = local_types;
                    }
                }
            }
            "field_declaration" => {
...
fn normalize_java_type(t: &str) -> String {
...
fn extract_java_local_types(
    node: &AstNode,
    current_scope_end: usize,
    type_map: &mut HashMap<String, Vec<(usize, usize, String)>>,
) {
    let node_type = match &node.kind {
        NodeKind::Unknown(t) => t.as_str(),
        _ => "",
    ...
    let active_scope_end = match node_type {
        "block" | "catch_clause" | "for_statement" | "enhanced_for_statement" | "try_statement" => node.span.end_line,
        _ => current_scope_end,
    };
    ...
    if node_type == "local_variable_declaration" {
        ...
        for child in &node.children {
            ...
            if child_type.contains("type") || child_type.contains("identifier") {
                type_name = normalize_java_type(&child.raw);
            } else if child.kind == NodeKind::Identifier {
                type_name = normalize_java_type(&child.raw);
            } else if child.kind == NodeKind::VariableDeclarator {
        ...
        for decl in declarators {
            let name = decl
                ...
                .map(|c| c.raw.clone())
                .unwrap_or_else(|| "unknown".to_string());
            type_map.entry(name).or_default().push((node.span.start_line, active_scope_end, type_name.clone()));
        }
    } else if node_type == "enhanced_for_statement" {
        let mut type_name = "Unknown".to_string();
        let mut var_name = "unknown".to_string();
        for child in &node.children {
            let child_type = match &child.kind {
                NodeKind::Unknown(t) => t.as_str(),
                _ => "",
            };
            if child_type == "formal_parameter" {
                let parts: Vec<&str> = child.raw.split_whitespace().collect();
                if parts.len() == 2 {
                    type_name = normalize_java_type(parts[0]);
                    var_name = parts[1].to_string();
                }
            }
        }
        if var_name != "unknown" {
            type_map.entry(var_name).or_default().push((node.span.start_line, active_scope_end, type_name));
        }
        for child in &node.children {
            extract_java_local_types(child, active_scope_end, type_map);
        }
        ...
        for child in &node.children {
            ...
            if child_type == "catch_formal_parameter" {
                let parts: Vec<&str> = child.raw.split_whitespace().collect();
                if parts.len() == 2 {
                    type_name = normalize_java_type(parts[0]);
                    var_name = parts[1].to_string();
