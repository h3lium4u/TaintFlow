...
impl<'a> InterproceduralTaintEngine<'a> {
                    ...
                    InstructionKind::Call { callee, args, .. } => {
                            ...
                            self.resolve_callee_info(node.method_id, callee)
                        {
                            ...
                            if let Some(stub) = stub_lookup {
                                if stub.kind == crate::stubs::StubKind::Sink {
                                    let mut resolved_to_strong = false;
                                    if class_fqn == "javax.crypto.Cipher" {
                                        if let Some(receiver_base) = get_receiver_name(callee) {
                                            if let Some(algo) = self.resolve_algorithm_from_receiver(node.method_id, &receiver_base) {
                                                if is_strong_algorithm(&algo) {
                                                    resolved_to_strong = true;
                                                }
                                            }
                                        }
                                    }

                                    if resolved_to_strong {
                                        is_sink = false;
                                    } else {
                                        is_sink = true;
                                        target_cwe = if class_fqn == "javax.crypto.Cipher" {
                                            crate::CWE::CWE327
                                        } else if class_fqn == "java.security.MessageDigest" {
                                            crate::CWE::CWE328
                                        } else {
                                    ...
                                    if is_sink && target_cwe == crate::CWE::CWE502 {
                        ...
                        if !is_sink {
                                ...
                                || callee_lower.contains("batchprocess")
                            {
                                ...
                                } else if clean_method_lower == "addcookie" {
                                    if let Some(cookie_var) = args.first() {
                                        if self.is_cookie_secure(node.method_id, cookie_var) {
                                            is_sink = false;
                                        } else {
                                            target_cwe = crate::CWE::CWE614;
                                        }
                                    } else {
                                        target_cwe = crate::CWE::CWE614;
                                    }
    ...
    fn get_library_call_return_type(&self, class_fqn: &str, method_name: &str) -> Option<String> {
        ...
        if (class_lower.contains("cipher") || class_lower.contains("javax.crypto.cipher")) && method_lower == "getinstance" {
            return Some("javax.crypto.Cipher".to_string());
        }
        if (class_lower.contains("messagedigest") || class_lower.contains("java.security.messagedigest")) && method_lower == "getinstance" {
            return Some("java.security.MessageDigest".to_string());
        }
        None
    }
    ...
    pub fn resolve_algorithm_from_receiver(&self, method_id: ir::MethodId, receiver_base: &str) -> Option<String> {
        let method = self.program.methods.get(&method_id)?;
        let mut all_insts = Vec::new();
        ...
        for inst_id in &all_insts {
            if let Some(inst) = self.program.instructions.get(inst_id) {
                if let ir::InstructionKind::Call { dest: Some(d), callee, args } = &inst.kind {
                    if d == receiver_base && callee.contains("getInstance") {
                        if let Some(first_arg) = args.first() {
                            target_var = Some(first_arg.clone());
                            break;
                        }
                    }
                }
        ...
        let var_name = target_var?;
        let var_trimmed = var_name.trim();

        if (var_trimmed.starts_with('"') && var_trimmed.ends_with('"'))
            || (var_trimmed.starts_with('\'') && var_trimmed.ends_with('\''))
        {
            return Some(var_trimmed[1..var_trimmed.len()-1].to_string());
        }
                ...
                match &inst.kind {
                    ir::InstructionKind::Assign { dest, src } => {
                        if dest == var_trimmed {
                            ...
                            {
                                return Some(src_trimmed[1..src_trimmed.len()-1].to_string());
                            }
                        }
                    }
                    ir::InstructionKind::Call { dest: Some(d), callee, args } => {
                        if d == var_trimmed {
                            let callee_lower = callee.to_lowercase();
                            if callee_lower.contains("getproperty") {
                                if args.len() >= 2 {
                                    let default_val = args[1].trim();
                                    if (default_val.starts_with('"') && default_val.ends_with('"'))
                                        || (default_val.starts_with('\'') && default_val.ends_with('\''))
                                    {
                                        return Some(default_val[1..default_val.len()-1].to_string());
                                    }
                                }
                            }
                        }
    ...
    pub fn is_cookie_secure(&self, method_id: ir::MethodId, cookie_var: &str) -> bool {
        let method = match self.program.methods.get(&method_id) {
            Some(m) => m,
            None => return false,
        };
        ...
        let mut set_secure = false;
        let mut set_httponly = false;
        
        for inst_id in &all_insts {
            if let Some(inst) = self.program.instructions.get(inst_id) {
                if let ir::InstructionKind::Call { dest: _, callee, args } = &inst.kind {
                    let callee_lower = callee.to_lowercase();
                    if let Some(receiver) = get_receiver_name_safe(callee) {
                        if receiver.trim() == cookie_var.trim() {
                            if callee_lower.contains("setsecure") {
                                if let Some(arg) = args.first() {
                                    if arg.trim().to_lowercase() == "true" {
                                        set_secure = true;
                                    }
                                }
                            }
                            if callee_lower.contains("sethttponly") {
                                if let Some(arg) = args.first() {
                                    if arg.trim().to_lowercase() == "true" {
                                        set_httponly = true;
                                    }
...
fn is_strong_algorithm(algo: &str) -> bool {
    let a = algo.to_lowercase();
    let clean = a.trim().replace('_', "-");
    if clean.starts_with("aes") {
        return true;
    }
    if clean.contains("sha-256") || clean.contains("sha256") {
        return true;
    }
    if clean.contains("sha-384") || clean.contains("sha384") {
        return true;
    }
    if clean.contains("sha-512") || clean.contains("sha512") {
        return true;
    }
    false
}